...
describe("KeyHandler BIP-39 (sdk-node)", () => {
  ...
  it("legacy mode always returns a full 32-byte private key hex, never a stripped one", async () => {
    // Same underlying node:crypto quirk as the padding fix in crypto.ts -
    // guard against it regressing back into this method specifically.
    for (let i = 0; i < 200; i++) {
      const key = await handler.restoreBIP39Key("k", `${TEST_PHRASE} ${i}`, { legacy: true });
      expect(key.key.prv.pkcs8pem).toMatch(/^0x[0-9a-f]{64}$/);
    }
  });
  ...
  it("BIP-32 master-key derivation matches the official BIP-32 test vector 1", async () => {
    // https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki#test-vector-1
    const crypto = require("crypto");
    const seed = Buffer.from("000102030405060708090a0b0c0d0e0f", "hex");
    const master = crypto.createHmac("sha512", "Bitcoin seed").update(seed).digest().subarray(0, 32);
    expect(master.toString("hex")).toBe("e8f32e723decf4051aefac8e2c93c9c5b214313817cdb01a1494b917c8436b35");
  });
});
