...
// correct.
//
// It also guards the one change that would be silent and unrecoverable: the
// secp256k1 phrase derivation is already shipped in @activeledger/sdk-node
// and sdk-web, so phrases exist. Every "v1" secp256k1 vector here is checked
// against the live restoreBIP39Key, which means a change to that derivation
// fails the build rather than quietly reassigning someone's identity.
...
const { KeyHandler } = require("../packages/node/lib/index.js");
...
// The backward-compatibility guard. Phrases generated by the shipped
// restoreBIP39Key must keep resolving to the same identity, so the vectors
// are checked against it rather than against a second copy of the formula.
await Promise.all(
  doc.phraseVectors
    .filter((v) => v.type === "secp256k1")
    .map(async (v) => {
