 ...
 * the @activeledger scope. Plenty of @activeledger packages live in other
 * repositories - rewriting one of those to this monorepo's version would
 * pin a dependency to a release that does not exist, and the failure would
 * only appear at install time on someone else's machine.
 */
...
if (!version || !/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(version)) {
  console.error("usage: set-version.mjs <version>   e.g. 2.1.1");
  process.exit(1);
}

const readJson = (p) => JSON.parse(readFileSync(p, "utf8"));
// Trailing newline: npm writes one, and without it every release would
// show a spurious no-newline-at-end-of-file diff on every manifest.
